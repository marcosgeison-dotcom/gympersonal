...
import {
  ChevronLeft, Sparkles, Play, Check, Plus, Minus, Dumbbell, Flame,
  Loader2, CheckCircle2, Zap, Home,
} from "lucide-react";
import { TODAY_WORKOUT, WORKOUT_PLANS } from "../mock";

export default function Workout() {
  const nav = useNavigate();
  const [tab, setTab] = useState("hoje");
  const [exercises, setExercises] = useState(
    TODAY_WORKOUT.exercises.map((e) => ({ ...e, logged: Array(e.sets).fill(null).map(() => ({ weight: e.weight, reps: 10, done: false })) }))
  );
  ...
  const updateSet = (ei, si, field, val) => {
    setExercises((prev) => prev.map((e, i) => i !== ei ? e : {
      ...e, logged: e.logged.map((s, j) => j !== si ? s : { ...s, [field]: Math.max(0, val) }),
    }));
  };
  const toggleSet = (ei, si) => {
    setExercises((prev) => prev.map((e, i) => i !== ei ? e : {
      ...e, logged: e.logged.map((s, j) => j !== si ? s : { ...s, done: !s.done }),
    }));
  };

  const generate = () => {
    setGenerating(true);
    setTimeout(() => { setGenerating(false); setTab("hoje"); }, 2200);
  };
    ...
    <div className="animate-slide-up" style={{ padding: "8px 16px 24px" }}>
      ...
      <div style={{ display: "flex", background: "#141418", borderRadius: 14, padding: 4, marginBottom: 16, border: "1px solid var(--border)" }}>
        {[["hoje", "Treino de Hoje"], ["planos", "Meus Planos"]].map(([k, l]) => (
          <button key={k} onClick={() => setTab(k)} style={{
            ...
            background: tab === k ? "linear-gradient(145deg,#7c5cff,#5b3ee0)" : "transparent",
      ...
      {tab === "hoje" && !finished && (
        <>
          ...
          <div className="card-surface" style={{ padding: 16, marginBottom: 14 }}>
            <div className="term-label neon-text">{TODAY_WORKOUT.day} · {TODAY_WORKOUT.focus}</div>
            <div className="font-display" style={{ fontSize: 24, fontWeight: 700, margin: "2px 0 12px" }}>{TODAY_WORKOUT.name}</div>
                ...
                <div key={ex.id} className="card-surface" style={{ overflow: "hidden", border: allDone ? "1px solid rgba(57,255,20,0.4)" : "1px solid var(--border)" }}>
                  <button onClick={() => setOpen(open === ei ? null : ei)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, padding: 12 }}>
                    ...
                    {allDone
                      ...
                      : <div className="font-mono-t" style={{ fontSize: 12, color: "var(--muted)" }}>{ex.logged.filter(s=>s.done).length}/{ex.sets}</div>}
                  </button>
                  {open === ei && (
                    <div style={{ padding: "0 12px 12px" }}>
                      {ex.logged.map((s, si) => (
                        <div key={si} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderTop: "1px solid var(--border)" }}>
                          ...
                          <button onClick={() => toggleSet(ei, si)} style={{
                            ...
                            background: s.done ? "#39ff14" : "#232329",
                      ...
                      <div className="font-mono-t" style={{ fontSize: 10, color: "var(--muted)", marginTop: 6 }}>
                        ...
                        IA sugere: aumentar carga se completar todas as reps
                      </div>
                    </div>
                  )}
                </div>
          ...
          <button onClick={() => setFinished(true)} style={primaryBtn}>
            <Check size={20} /> Finalizar Treino
          </button>
          <button style={{ ...ghostBtn, marginTop: 10 }}>
            <Home size={18} /> Treino Emergencial (sem equipamento)
          </button>
        </>
      )}
      ...
      {tab === "hoje" && finished && (
        <div className="card-surface animate-slide-up" style={{ padding: 24, textAlign: "center", marginTop: 20 }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 13, margin: "0 0 16px" }}>Você ganhou <span className="neon-text" style={{ fontWeight: 700 }}>+280 XP</span> e manteve seu streak 🔥</p>
      ...
      {tab === "planos" && (
        <>
          <button onClick={generate} disabled={generating} style={{ ...primaryBtn, marginBottom: 16 }}>
            {generating ? <><Loader2 size={20} className="spin-loader" style={{ animation: "spin 1s linear infinite" }} /> Gerando treino IA...</> : <><Sparkles size={20} /> Gerar Plano com IA</>}
          </button>
            ...
            {WORKOUT_PLANS.map((p) => (
              <div key={p.id} className="card-surface" style={{ padding: 14, border: p.active ? "1px solid rgba(124,92,255,0.5)" : "1px solid var(--border)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  {p.active
                    ? <span className="term-label neon-text" style={{ padding: "4px 8px", background: "rgba(124,92,255,0.15)", borderRadius: 8 }}>ATIVO</span>
                    : <button style={{ ...ghostBtn, width: "auto", padding: "7px 14px", fontSize: 12 }}>Ativar</button>}
                </div>
...
const primaryBtn = {
  ...
  background: "linear-gradient(145deg,#7c5cff,#5b3ee0)", color: "#fff",
...
const ghostBtn = {
  ...
  background: "transparent", color: "#c9c9d4", border: "1px solid var(--border)",
  fontFamily: "Rajdhani", fontWeight: 600, fontSize: 14,
  display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
};

...
import { EXERCISE_LIBRARY, MUSCLE_GROUPS } from "../mock";
      ...
      <div className="no-scrollbar" style={{ display: "flex", gap: 8, overflowX: "auto", marginBottom: 16, paddingBottom: 2 }}>
        {MUSCLE_GROUPS.map((g) => (
          <button key={g} onClick={() => setGroup(g)} style={{
            whiteSpace: "nowrap", padding: "7px 14px", borderRadius: 20, cursor: "pointer", fontSize: 12, fontWeight: 600,
            border: group === g ? "1px solid var(--neon)" : "1px solid var(--border)",
            background: group === g ? "rgba(124,92,255,0.16)" : "#141418",
            color: group === g ? "var(--neon)" : "var(--muted)",
          }}>{g}</button>
        ))}
      </div>

      <div className="font-mono-t" style={{ fontSize: 11, color: "var(--muted)", marginBottom: 10 }}>{filtered.length} exercícios</div>
